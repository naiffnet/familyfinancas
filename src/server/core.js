...
function createOwnershipChecks(db) {
  return {
    ...
    'dashboard:getGeneralSummary': (session, d) => isSameFamilyUser(db, d ? d.userId : null, session),
    'dashboard:getMonthlyChart': (session, d) => isSameFamilyUser(db, d ? d.userId : null, session),
    'dashboard:getCategoryChart': (session, d) => isSameFamilyUser(db, d ? d.userId : null, session),
    'reports:getCashflow': (session, d) => isSameFamilyUser(db, d ? d.userId : null, session),
    'reports:getPatrimony': (session, d) => isSameFamilyUser(db, d ? d.userId : null, session),
    'reports:getInterestAudit': (session, d) => isSameFamilyUser(db, d ? d.userId : null, session),
    ...
    'cards:getAdvanceableInstallments': (session, d) => isSameFamilyUser(db, (typeof d === 'object' && d !== null ? (d.userId || d.id) : null) || session.userId, session.familyId),
    'cards:advanceInstallments': (session, d) => isSameFamilyUser(db, d.userId, session.familyId),
    'accounts:reconcileOfx': (session, d) => isSameFamilyUser(db, d.userId, session.familyId),
...
function buildHandlers(db) {
  return {
    ...
    'cards:getAdvanceableInstallments': (d) => db.getAdvanceableInstallments(typeof d === 'object' ? (d.cardAccountId || d.id) : d),
    'cards:advanceInstallments': (d) => db.advanceInstallments(d),
    'accounts:reconcileOfx': (d) => db.reconcileOfxTransactions(d),
